...
c = 299792.458
#gal_radial_vels = [701.0, 655, 1516.2, 1570.5, 1637.1,1498.1, 1074.2, 833,1506.1,1335.2,885.2, 766, 702, 2399, 1564, 1570, 1951, 668.5, 1647.9 ]
#vel_offset_means = [2.29327893,  4.17833797,17.42341014, 29.95852831, 26.81590357, 23.93995921,  18.08730675, -33.57676859,  26.86411143, 19.53263818,  22.38940624,  -7.98805774,  -14.08827485,  10.12162512,  3.24927299,  -1.2154357,  -2.42323438,  2.42687086,  8.17597689]
#print(len(vel_offset_means))

#gal_radial_vels = np.array(gal_radial_vels) - vel_offset_means
...
def extract_info(gmc_catalog, gmc_catalog_version, muse, matching, outliers, threshold_perc, vel_limit, randomize, symmetrical):
    ...
    def matching1om(hiis):
        ...
        idoverhii = [int(hii.get("region_ID"))  for hii in hiis]
        idovergmc = [undo_list(hii.get("GMCS"))  for hii in hiis]

        idrejectedhii = [int(hii.get("region_ID"))  for hii in hiireject]
        idrejectedgmc = [undo_list(hii.get("GMCS"))  for hii in hiireject]
    ...
    def overlap_percent(rahii, dechii, major_gmc, minor_gmc, angle_gmc, decgmc, ragmc, radiushii, radgmc, header,
        ...
        if os.path.isfile((dir_script_data+"raw_%s%s%s.pickle" % (name_unmatched, namegmc, galnam,))) and overwrite == False:
            ...
            j = 0
            for hii in hiis:  # hiis_loop:  # range(len(ragmc)):  # for the gmc regions that passed the precedent condition
                hii["GMCS"] = []
                hii["OVERLAP_PIX"] = []
                hii["DELTAV"] = []
                ...
                if hii in hiis_loop:
                    ...
                    for gmc in gmcs:  # gmcs_loop:#gmcs1:
                        ...
                        if gmc in gmcs_loop:
                            mask_gmc_all = copy.deepcopy(co_mask)
                            mask_gmc_all[0].data[np.where(mask_gmc_all[0].data.astype(int) != gmc['CLOUDNUM'])] = 0
                            mask_gmc_all[0].data[np.where(mask_gmc_all[0].data.astype(int) == gmc['CLOUDNUM'])] = 1
    ...
    def vel_filter(hiis, gmcs, vel):
        ...
        for hii in hiis:
            ...
            indexes_hii = extract_ind_inf([x.value - gal_radial_vel for x in hii['DELTAV']], vel)
