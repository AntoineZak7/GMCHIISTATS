...
def plot_single_correlations(muse, gmc_catalog, gmc_catalog_version, randomize, matching, outliers,  show, save, threshold_percs, vel, gmcprop, rgal_color):
    # ===============================================================
    ...
    for threshold_perc in threshold_percs:
        ...
        for i in gmcprop:
            ...
            fig, axs = plt.subplots(1, 1, sharex='col', figsize=(10, 11), dpi=80, gridspec_kw={'hspace': 0})
            #plt.subplots_adjust(wspace=0.3)
            #plt.subplots_adjust(hspace=0.0)

            name_gmc_prop = ['','MCO','','Sigma_mol','Sigma_v','','CO_Tpeak']
            pdf_name = "%sCor_HA_%s%s%s.pdf" % (dirplots, name_gmc_prop[i], name_end, str(z))


            pdf3 = fpdf.PdfPages(pdf_name)

            # axs.set_ylabel(labsyay[i], fontsize = 18)
            # axs.grid()
            # axs.set_xlabel(labsxax[1], fontsize = 18)
            #
            ...
            axs.plot(xaxall, yayall,'o', markerfacecolor = 'None' ,color = 'black', markersize = 2, label='threshold = %f' % threshold_perc ,linestyle = 'None')
            ...
            if xaxall.any() != 0 and yayall.any != ():
                ...
                axs.text(0.8, 0.03, 'R²: %5.3f' % ((r_sq)**2), fontsize=10, horizontalalignment='center',
                            verticalalignment='center', transform=axs.transAxes)

                # axs.text(0.8, 0.19, 'Durbin-Watson stat: %6.2f' % (dw), fontsize=8, horizontalalignment='center',
                #             verticalalignment='center', transform=axs.transAxes)

                axs.text(0.1, 0.03, 'Slope %5.3f ' % (slope), fontsize=10, horizontalalignment='center',
                            verticalalignment='center', transform=axs.transAxes)


                #
                axs.text(0.8, 0.07, 'Standard deviation %5.3f ' % (np.sqrt(MSe)), fontsize=10, horizontalalignment='center',
                            verticalalignment='center', transform=axs.transAxes)


                axs.text(0.8, 0.11, 'paired gmcs %5.3f (%5.2f %%) ' % (n_gmcs, n_gmcs*100/n_gmcs_tot), fontsize=10, horizontalalignment='center',
                         verticalalignment='center', transform=axs.transAxes)

                axs.text(0.8, 0.15, 'paired hii regions %5.3f (%5.2f %%) ' % (n_hiis, n_hiis*100/n_hiis_tot), fontsize=10, horizontalalignment='center',
                         verticalalignment='center', transform=axs.transAxes)
    ...
    for threshold_perc in threshold_percs:
        ...
        for i in gmcprop:
            ...
            if xaxall.any() != 0 and yayall.any != ():
                ...
                #reg= LinearRegression().fit(xaxall.reshape(-1,1),yayall)
                test_a, test_b, test_r, tst_t, see = scipy.stats.linregress(xaxall,yayall)
                test_r = test_r**2
                mx = xaxall.mean()
                sx2 = ((xaxall - mx) ** 2).sum()
                sd_intercept = see * np.sqrt(1. / len(xaxall) + mx * mx / sx2)
                sd_slope = see * np.sqrt(1. / sx2)

                x_test = copy.deepcopy(xaxall)
                x_test = sm.add_constant(x_test)

                model = sm.OLS(yayall, x_test)
                results = model.fit()
                print(results.summary())


                print("Regression antoine: slope = %5.2f $\pm$ %5.2f,  intercept = %5.2f $\pm$%5.2f, r² = %5.2f" %(slope, conf_a, b, conf_b, r_sq))
                print("Regression scikit: slope = %5.2f $\pm$ %5.2f,  intercept = %5.2f $\pm$ %5.2f, r² = %5.2f" %(test_a, results.params[1], test_b, results.params[0], test_r))
                ...
                axs[l].text(0.58, 0.19, 'R²: %5.3f' % ((r_sq)**2), fontsize=text_size_mult, horizontalalignment='left',
                            verticalalignment='center', transform=axs[l].transAxes, weight = 'bold')

                axs[l].text(0.58, 0.12, 'Intercept: %5.2f $\pm$%5.1f' % (b, conf_b), fontsize=text_size_mult, horizontalalignment='left',
                            verticalalignment='center', transform=axs[l].transAxes, weight = 'bold')

                # axs.text(0.8, 0.19, 'Durbin-Watson stat: %6.2f' % (dw), fontsize=8, horizontalalignment='center',
                #             verticalalignment='center', transform=axs.transAxes)

                axs[l].text(0.58, 0.05, 'Slope %5.3f $\pm$%5.2f' % (slope, conf_a), fontsize=text_size_mult, horizontalalignment='left',
                            verticalalignment='center', transform=axs[l].transAxes, weight = 'bold')
                ...
                if l == 0:
                    axs[l].legend(prop={'size': 14.5}, loc = 2)
...
def plot_correlations_regions(muse, gmc_catalog, gmc_catalog_version, randomize, matching, outliers,  show, save, threshold_percs, vel, gmcprop, rgal_color, regions, symmetrical):
    ...
    if save == True:
        pdf_name = "%sCorrelations_HA_%s_threshold%s.pdf" % (dirplots,labsyay[gmcprop[0]],  name_end)
    ...
    for i in range(len(regions)):
        ...
        for i in gmcprop:
            ...
            axs.plot(xaxall, yayall,'o', markerfacecolor = 'None' , markersize = 2, markeredgecolor = 'black', label='threshold = %2.0f\%%' % (threshold_perc*100) ,linestyle = 'None')
            ...
            if xaxall.any() != 0 and yayall.any != ():
                ...
                if regions_id != [7,8,9]:
                    ...
                    axs.plot(x0[indexes], y_pred[indexes] - int_prev_y[indexes], '-.', color = 'grey')
                    axs.plot(x0[indexes_1], y_pred[indexes_1] - int_prev_y[indexes_1], '-.', color = 'grey', alpha = 0.3)
                ...
                axs.text(left_align, down_align + 3*offset, 'R²: %5.3f' % ((r_sq)**2), fontsize=text_size_r2slope, horizontalalignment='left',
                            verticalalignment='center', transform=axs.transAxes)

                axs.text(left_align, down_align + offset, 'Intercept %5.3f $\pm$ %5.2f ' % (b, conf_b), fontsize=text_size_r2slope, horizontalalignment='left',
                            verticalalignment='center', transform=axs.transAxes)

                axs.text(left_align, down_align, 'Slope %5.3f $\pm$ %5.2f ' % (slope, conf_a), fontsize=text_size_r2slope, horizontalalignment='left',
                            verticalalignment='center', transform=axs.transAxes)


                axs.text(left_align, down_align + 2*offset, 'Std: %5.3f ' % (np.sqrt(MSe)), fontsize=text_size_r2slope, horizontalalignment='left',
                            verticalalignment='center', transform=axs.transAxes)


                axs.text(right_align, down_align , 'matched gmcs: %5.0f (%5.2f \%%) ' % (n_gmcs, (n_gmcs*100/n_gmcs_tot)), fontsize=text_size, horizontalalignment='left',
                         verticalalignment='center', transform=axs.transAxes)

                axs.text(right_align, down_align + 1*offset, 'matched hii regions: %5.0f (%5.2f \%%) ' % (n_hiis, (n_hiis*100/n_hiis_tot)), fontsize=text_size, horizontalalignment='left',
                         verticalalignment='center', transform=axs.transAxes)

                axs.text(right_align, down_align + 2*offset, 'matched gmcs CO flux: %5.2f \%% ' % (FluxCOGMCovertot*100/FluxCOGMCtot),
                         fontsize=text_size, horizontalalignment='left',
                         verticalalignment='center', transform=axs.transAxes)

                axs.text(right_align, down_align + 3*offset, 'matched hii regions Ha Lum: %5.2f \%% ' % (LumHacorrovertot*100/LumHacorrtot),
                         fontsize=text_size, horizontalalignment='left',
                         verticalalignment='center', transform=axs.transAxes)
    ...
    for i in range(len(arrayyay1)):
        ...
        for thres in threshold_percs:
            ...
            if i == 3 or i == 5:
                ...
                axs[l].text(0.05, 0.90, 'Mean all %5.2f ' % (mean), fontsize=fontsize,
                                horizontalalignment='left',
                                verticalalignment='center', transform=axs[l].transAxes, weight = 'bold')

                axs[l].text(0.05, 0.62, 'Median all %5.2f ' % (median), fontsize=fontsize,
                                horizontalalignment='left',
                                verticalalignment='center', transform=axs[l].transAxes, weight = 'bold')

                axs[l].text(0.05, 0.34, 'Std all %5.2f ' % (standard_dev), fontsize=fontsize,
                        horizontalalignment='left',
                        verticalalignment='center', transform=axs[l].transAxes, weight = 'bold')
            ...
            if  yayall.any != ():
                ...
                axs[l].hist(yayall,  bins = 100, color = 'blue', label = 'matched gmcs \n (thres = %2.0f \%%)' %(100*thres),  histtype='stepfilled', alpha = 0.65, stacked = True, density = True)
            ...
            if i == 3 or i == 5:
                ...
                axs[l].text(0.05, 0.78, 'Mean matched %5.2f ' % (mean), fontsize=fontsize,
                                horizontalalignment='left',
                                verticalalignment='center', transform=axs[l].transAxes, weight = 'bold')

                axs[l].text(0.05, 0.50, 'Median matched %5.2f ' % (median), fontsize=fontsize,
                                horizontalalignment='left',
                                verticalalignment='center', transform=axs[l].transAxes, weight = 'bold')

                axs[l].text(0.05, 0.22, 'Std matched %5.2f ' % (standard_dev), fontsize=fontsize,
                        horizontalalignment='left',
                        verticalalignment='center', transform=axs[l].transAxes, weight = 'bold')
...
def hist_std_hii_props(muse, gmc_catalog, matching, outliers, gmc_catalog_version, randomize,vel,  show, save, threshold_percs, bin, gmc_props):
    ...
    for i in range(len(arrayyay1)):
        ...
        for thres in threshold_percs:
            ...
            labsxax, labsyay, arrayxax, arrayyay, name_end, namegmc, galaxias, idoverhii, idovergmc, LumHacorrnot, FluxCOGMCnot, FluxCOGMCover, regionindexGMCover, HIImajor, majorGMC, minorGMC, MassCOGMC, SizepcGMC, SizepcHII, MasscoGMCover, SizepcGMCover, SigmaMol, Sigmav, COTpeak = get_data(
                matching=matching, muse=muse, gmc_catalog=gmc_catalog, gmc_catalog_version=gmc_catalog_version,
                outliers=outliers, randomize=randomize, threshold_perc=thres, vel=vel)

            arrayyay1 = [arrayxax[0], arrayxax[1]]


            axs[2].set_xlabel(labsyay1[i])#, fontsize = 20)
            #axs[l].tick_params(axis="both", labelsize=18, reset=False)
            #axs[l].grid()
            yaytmp = arrayyay1[i]
            ...
            if yayall_all.any != ():
                 axs[l].hist(yayall_all, bins = binn, color = 'red', label = 'all hii', histtype='stepfilled',density = True, alpha = 0.7)
                 axs[0].legend(prop={'size': 4}, loc=2)
            ...
            axs[l].text(left_align, 0.90-0.12*2-0.14*2, 'Std all %5.2f ' % (standard_dev), fontsize=fontsize,
                        horizontalalignment='left',
                        verticalalignment='center', transform=axs[l].transAxes, weight='bold')
            ...
            if  yayall.any != ():
                ...
                axs[l].hist(yayall,  bins = binn, color = 'blue', label = 'paired hii  \n (thres = %2.0f \%%)' %(thres*100), histtype='stepfilled',density = True, alpha = 0.65)
            ...
            axs[l].text(left_align, 0.90-0.12-0.12-0.14, 'Median matched %5.3f ' % (median), fontsize=fontsize,
                        horizontalalignment='left',
                        verticalalignment='center', transform=axs[l].transAxes, weight = 'bold')

            axs[l].text(left_align, (0.90-0.12-0.12-0.14-0.14-0.12), 'Std matched %5.2f ' % (standard_dev), fontsize=fontsize,
                        horizontalalignment='left',
                        verticalalignment='center', transform=axs[l].transAxes, weight='bold')
...
def cdf_std_props(muse, gmc_catalog, matching, outliers, gmc_catalog_version, randomize,vel,  show, save, threshold_percs, bin, gmc_props, symmetrical):
    ...
    for i in range(len(arrayyay1)):
        ...
        if yayall_all.any != ():
            ...
            X = X / np.nanmax(X)
            axs.plot(sorted(Y), X, color='red', label='all gmcs', linewidth = 0.6)
...
#plot_correlations(muse='dr2', gmc_catalog="_native_", gmc_catalog_version='new', matching="overlap_1om", outliers=True, show=False, save=True, vel=10000, threshold_percs=[0.1,0.5,0.9], randomize='', gmcprop=[3,6], rgal_color=False, symmetrical='')
#plot_correlations_regions(muse='dr2', gmc_catalog="_native_", gmc_catalog_version='new', matching="overlap_1om", outliers=True, show=False, save=True, vel=10000, threshold_percs=[0.9], randomize='', gmcprop=[4], regions = regions, rgal_color=False, symmetrical='')
...
#hist_std_hii_props(muse = 'dr2', gmc_catalog_version = 'new', gmc_catalog='_native_', matching= 'overlap_1om', outliers=True, randomize='', vel = 100, show = False, save = True,threshold_percs=[0.1,0.5,0.9], gmc_props=[1], bin = 100, symmetrical = '')
...
#cdf_std_hii_props(muse = 'dr2', gmc_catalog_version = 'new', gmc_catalog='_native_', matching= 'overlap_1om', outliers=True, randomize='', vel = 100, show = False, save = True,threshold_percs=[0.1,0.5,0.9], gmc_props=[1], bin = 100, symmetrical = '')
...
#plot_correlations(muse='dr2', gmc_catalog="_native_", gmc_catalog_version='new', matching="overlap_1om", outliers=True, show=False, save=True, vel=10000, threshold_percs=[0.9], randomize='', gmcprop=[1,4], rgal_color=False, symmetrical='')
